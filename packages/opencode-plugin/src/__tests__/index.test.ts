...
vi.mock('@opencode-ai/plugin', () => {
  ...
  const createChainableSchema = (): Record<string, Function> => {
  ...
  return {
    tool: Object.assign(
      (config: { description: string; args: unknown; execute: Function }) => ({
...
describe('@parallel-web/opencode-plugin exports', () => {
  ...
  describe('plugin function', () => {
    it('should return plugin config when called', async () => {
      const { ParallelWebPlugin } = await import('../index.js');
      const mockContext = {};
      const pluginConfig = await ParallelWebPlugin(mockContext as any);
