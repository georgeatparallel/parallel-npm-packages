...
async function resolveParallelCliPath(): Promise<string> {
  ...
  if (!binRelativePath) {
    throw new Error("Could not resolve parallel-cli binary from parallel-web-cli package.");
  }
...
export async function runParallelCli(args: string[], options: RunParallelCliOptions = {}): Promise<RunParallelCliResult> {
...
export async function runParallelCliJson<T>(args: string[], options: RunParallelCliOptions = {}): Promise<T> {
  const result = await runParallelCli(args, options);

  if (result.exitCode !== 0) {
    const authMessage = result.exitCode === 3
